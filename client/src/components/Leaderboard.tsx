...
export function Leaderboard({ className }: LeaderboardProps) {
  ...
  const { leaderboard, playerName, getTopResults, getPlayerHistory, clearLeaderboard, updatePlayerName } = useLeaderboard();
  ...
  const handleClearLeaderboard = () => {
    if (confirm('Вы уверены, что хотите очистить таблицу рейтингов?')) {
      clearLeaderboard();
      toast.success('Таблица рейтингов очищена');
    }
  };
      ...
      <DialogContent className="sm:max-w-lg max-w-[95vw] max-h-[85vh] overflow-hidden flex flex-col mx-auto">
        <DialogHeader className="flex-shrink-0">
          ...
          <DialogDescription className="text-sm">
            Лучшие результаты и история ваших игр
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex items-center gap-2 p-2 sm:p-3 bg-slate-50 rounded-lg flex-shrink-0">
          ...
          {editingName ? (
            ...
            <>
              <span className="flex-1 text-sm font-medium truncate">{playerName || 'Игрок'}</span>
              <Button 
                variant="ghost" 
                size="sm" 
                onClick={() => {
        ...
        <Tabs defaultValue="top" className="flex-1 flex flex-col min-h-0">
          <TabsList className="grid w-full grid-cols-2 flex-shrink-0">
            <TabsTrigger value="top" className="text-sm">Топ-10</TabsTrigger>
            <TabsTrigger value="history" className="text-sm">Моя история</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="top" className="flex-1 overflow-auto mt-2">
            {topResults.length === 0 ? (
              <div className="text-center py-8 text-slate-500">
                ...
                <p className="text-sm">Пока нет результатов</p>
                <p className="text-xs mt-1">Сыграйте, чтобы попасть в рейтинг!</p>
              </div>
            ) : (
          ...
          <TabsContent value="history" className="flex-1 overflow-auto mt-2">
            {playerHistory.length === 0 ? (
              <div className="text-center py-8 text-slate-500">
                ...
                <p className="text-xs mt-1">Ваши результаты появятся здесь</p>
              </div>
            ) : (
              <div className="space-y-2">
